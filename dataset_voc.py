...
class VOC_TrainDataset(torchdata.Dataset):
    ...
    def _get_sub_batch(self):
        while True:
            # get a sample record
            this_sample = self.list_images[self.cur_idx]
    ...
    def __getitem__(self, index):
        ...
        batch_segms = torch.zeros(self.batch_per_gpu, self.cropSize // self.segm_downsampling_rate,
                                  self.cropSize // self.segm_downsampling_rate).long()

        for i in range(self.batch_per_gpu):

            image_path = os.path.join(self._image_dir, batch_records[i] + ".jpg")
            label_path = os.path.join(self._mask_dir, batch_records[i] + ".png")
            ...
            img, target = self._sync_transform(img, target)
             ...
             # image to float
            img = np.array(img).astype(np.float32)[:, :, ::-1] # RGB to BGR!!!
            img = img.transpose((2, 0, 1))
            img = self.img_transform(torch.from_numpy(img.copy()))

            target = target.resize((int(self.cropSize/self.segm_downsampling_rate), int(self.cropSize / self.segm_downsampling_rate)), Image.NEAREST)
            target = np.array(target)

            batch_images[i][:, :, :] = img
            batch_segms[i][:, :] = torch.from_numpy(target.astype(np.int)).long()
            # batch_images[i][:, :img.shape[1], :img.shape[2]] = img
            # batch_segms[i][:target.shape[0], :target.shape[1]] = torch.from_numpy(target.astype(np.int)).long()
...
class VOC_TestDataset(torchdata.Dataset):
